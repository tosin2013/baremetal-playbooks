...
from trigger_equinix_server_instance import (
    init_db, get_defaults, save_defaults, trigger_github_action, update_github_secret, cli_main, gui_main
)
...
def test_get_defaults(mock_db_connection):
    mock_conn, mock_cursor = mock_db_connection
    mock_cursor.fetchone.return_value = (1, 'password', 'access_key', 'secret_key', 'host', 'username', 'domain', 'forwarder', 'fqdn', 'domain', 'guid', 'ollama')
    result = get_defaults()
    assert result == (1, 'password', 'access_key', 'secret_key', 'host', 'username', 'domain', 'forwarder', 'fqdn', 'domain', 'guid', 'ollama')

# Test save_defaults function
def test_save_defaults(mock_db_connection):
    mock_conn, mock_cursor = mock_db_connection
    defaults = ('password', 'access_key', 'secret_key', 'host', 'username', 'domain', 'forwarder', 'fqdn', 'domain', 'guid', 'ollama')
...
def test_cli_main(mock_trigger, mock_update):
    ...
    cli_main(args)
    mock_update.assert_any_call('tosin2013', 'baremetal-playbooks', 'SSH_PASSWORD', 'password', 'token')
    mock_update.assert_any_call('tosin2013', 'baremetal-playbooks', 'AWS_ACCESS_KEY', 'access_key', 'token')
    mock_update.assert_any_call('tosin2013', 'baremetal-playbooks', 'AWS_SECRET_KEY', 'secret_key', 'token')
    mock_trigger.assert_called_once_with(
        'tosin2013', 'baremetal-playbooks', 'equinix-metal-baremetal-blank-server.yml', 'token',
        {
            'NEW_HOST': 'host',
            'NEW_USERNAME': 'username',
            'NEW_DOMAIN': 'domain',
...
def test_gui_main(mock_trigger, mock_update, mock_getenv, mock_success, mock_button, mock_text_input):
    ...
    mock_update.assert_any_call('tosin2013', 'baremetal-playbooks', 'AWS_ACCESS_KEY', 'access_key', 'token')
    mock_update.assert_any_call('tosin2013', 'baremetal-playbooks', 'AWS_SECRET_KEY', 'secret_key', 'token')
    mock_update.assert_any_call('tosin2013', 'baremetal-playbooks', 'KCLI_PIPELINES_RUNNER_TOKEN', 'runner_token', 'token')
    mock_trigger.assert_called_once_with(
        'tosin2013', 'baremetal-playbooks', 'equinix-metal-baremetal-blank-server.yml', 'token',
        {
            'NEW_HOST': 'host',
